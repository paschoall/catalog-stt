function aceitarRecurso(id) {
	...
	if(confirm("Aceitar recurso \"" + titulo + "\"?")) {
		$.post(window.root + 'aceitar_recurso-db.php', {"id_recurso": id}).done(function(data) {
			data = data.trim();
...
$(function() {
	...
	for(var i = 0; i < window.requisicoes.length; ++i) {
		...
		titulo.append("<a style='float: right; font-size:11pt;' target='_blank' href='" + window.root + "info_recurso.php?id=" + window.requisicoes[i]['ID_RECURSO'] + "'>Ver recurso</a>"); 
		li.append(titulo);
		...
		var bot_recusar = $("<div class = 'col s6'> <center> <a id='recusar_" + window.requisicoes[i]['ID_RECURSO'] + "'target = '_blank' href='" + window.root + "recusarRecurso.php?id=" + window.requisicoes[i]['ID_RECURSO'] + "&email=" + window.requisicoes[i]['EMAIL'] + "' class='waves-effect waves-light btn red lighten'>FAZER UMA OBSERVAÇÃO</a> </center> </div>")
